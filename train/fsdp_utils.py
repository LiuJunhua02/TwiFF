...
class FSDPCheckpoint:
    ...
    def fsdp_save_ckpt(
        ...
        max_ckpt_to_keep: int = 2,  # 新增参数：最多保留多少个 checkpoint
    ):
        ...
        with FSDP.state_dict_type(
            ...
            if dist.get_rank() == 0:
                ...
                model_state_dict = _to_bf16(model_state_dict)
                save_file(model_state_dict, os.path.join(save_path, "model.safetensors"))
        ...
        # ====== clear the old checkpoint ======
        if dist.get_rank() == 0 and max_ckpt_to_keep > 0:
            ckpt_root = Path(ckpt_dir)
            # 获取所有 checkpoint 目录（假设命名格式为 0000001, 0000010 等数字）
            ckpt_dirs = sorted(
                [d for d in ckpt_root.iterdir() if d.is_dir() and d.name.isdigit()],
                key=lambda x: int(x.name)
            )
            # 如果数量超过上限，删除最旧的
            while len(ckpt_dirs) > max_ckpt_to_keep:
