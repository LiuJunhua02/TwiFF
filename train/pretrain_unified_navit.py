...
class TrainingArguments:
    ...
    save_only_model: bool = field(
        default=True,
        metadata={"help": "Only save the model checkpoint"}
    )
    checkpoint_num: int = field(
        default=1,
        metadata={"help": "Number of max checkpoints"}
    )
...
def main():
    ...
    if dist.get_rank() == 0:
        ...
        tensorboard_dir = os.path.join(training_args.results_dir, "tensorboard")
    ...
    for micro_step, data in enumerate(train_loader):
        ...
        if curr_step % training_args.log_every == 0:
            ...
            train_log['total_mse_tokens'] = total_mse_tokens.item()
            train_log['total_ce_tokens'] = total_ce_tokens.item()
            train_log['total_norm'] = total_norm.item()
            train_log['total_samples'] = total_samples.item()
            train_log['tokens_per_sec'] = tokens_per_sec
            train_log['tokens_per_step'] = tokens_per_step
            train_log['actual_tflops'] = actual_tflops
            train_log['mfu'] = mfu_value
        ...
        if curr_step > 0 and curr_step % training_args.save_every == 0:
            ...
            FSDPCheckpoint.fsdp_save_ckpt(
                ...
                max_ckpt_to_keep = training_args.checkpoint_num, 
            )
