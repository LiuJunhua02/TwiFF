...
import argparse
from eval.gencot.inferencer_batch import InterleaveInferencer
...
def get_args():
    ...
    parser.add_argument('--QA_file', default='/path/to/TwiFF-Bench', type=str)
    parser.add_argument('--output_dir', default='/path/to/benchmark_output', type=str)
    parser.add_argument('--root_path', default='/path/to/TwiFF-Bench_video/', type=str)
...
if __name__ == '__main__':
    ...
    print(f"[Rank {rank}] use TwiFF cot!")
    ...
    for idx in range(start, end):
        ...
        clip = ditem['frames']
        question_file = os.path.join(args.root_path, ditem["video"])
        image = read_frames_decord(question_file, num_frames=8, start_frames=1, end_frames=1, sample='middle', clip=clip)
        # image = [img.resize((512, 512)) for img in image]
        assert prompt.count('<image>') == len(image), f'question_{idx} missing the image placeholder!'
        ...
        output, is_end_list = inferencer.cot_inference(current_input, system_prompt=sys_prompt, max_round=args.max_round, **inference_hyper)
        ...
        for k in range(len(is_end_list)):
            ...
            for iteration, item in enumerate(output):
                if isinstance(item[k], str):
                    ...
                    image_filename = f'reasoning_{idx}_image_{iteration + 1}.png'
