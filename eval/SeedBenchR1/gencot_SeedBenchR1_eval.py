...
def get_args():
    ...
    parser.add_argument('--model_name', default='openai/gpt-5.1', type=str)
    parser.add_argument('--data_path', default='/path/to/data', type=str)
    parser.add_argument('--out_path', default='/path/to/output_score', type=str)
    parser.add_argument('--root_dir', default='/path/to/benchmark_img_dir', type=str)
    ...
    parser.add_argument('--num_frames', type=int, default=1, help="max image patch num")
    parser.add_argument('--start_frames', type=int, default=0, help="start frames")
    parser.add_argument('--end_frames', type=int, default=0, help="end frames")
    return parser.parse_args()

system_prompt_template = '''You are a evaluator. You will have to evaluate the model answer based on the question and ground truth answer.
Given:
...
def prepare_prompt(
                 question_file, #images of question
                 question, #question
                 answer,#groundtruth
    ...
    # user_content.append({"type": "text", "text": f"{model_answer}"})
...
def prepare_prompt_wrapper(data):
    try:
        ...
        question_file = [os.path.join(root_dir, 'images', data['video_source'], data['current_observation_basename'])]
        question_file = [Image.open(image_path) for image_path in question_file]
        options = data["answer"]
...
if __name__ == "__main__":
    ...
    client = AsyncOpenAI(base_url=VLLM_API_BASE, api_key=API_KEY,     http_client=httpx.AsyncClient(
        timeout=httpx.Timeout(300, read=600)  # connect:300s, read:600s
    ))
    asyncio.run(main())