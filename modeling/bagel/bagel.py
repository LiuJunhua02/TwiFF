...
class Bagel(PreTrainedModel):
    ...
    def generate_image(
        ...
        for i, t in tqdm(enumerate(timesteps), total=len(timesteps), disable=not use_tqdm):
    ...
    def generate_text(
        ...
        batch_size = packed_start_tokens.shape[0]
        ...
        step = 0
        generated_sequence = []
        curr_tokens = packed_start_tokens.clone()  # [batch_size]

        while step < max_length:
            generated_sequence.append(curr_tokens.clone())

            # Embed current tokens
            packed_text_embedding = self.language_model.model.embed_tokens(curr_tokens)  # [batch_size, hidden]

            # Prepare query-related tensors (all batch elements advance by 1 token)
            query_lens = torch.ones_like(curr_tokens)  # [batch_size], all 1
            cumsum_kv_lens = torch.cumsum(key_values_lens, dim=0)
            arange_batch = torch.arange(0, batch_size, device=key_values_lens.device, dtype=key_values_lens.dtype)
            packed_query_indexes = cumsum_kv_lens + arange_batch  # [batch_size]

            # Adjust packed_key_value_indexes: offset each sequence's indices
            ...
            pred_logits = self.language_model.lm_head(packed_query_sequence)  # [batch_size, vocab]
            ...
            if do_sample:
                ...
                next_tokens = torch.argmax(pred_logits, dim=-1)  # [batch_size]

            # Handle early stopping per sequence
            if end_token_id is not None:
                # For finished sequences, force output to be end_token_id
                next_tokens = torch.where(unfinished_sequences, next_tokens, torch.tensor(end_token_id, device=device))

                # Update unfinished mask
                newly_finished = (next_tokens == end_token_id)
            ...
            # Update cache indexing structures for next step
            uppacked = list(packed_key_value_indexes.split(key_values_lens.tolist(), dim=0))
            for i in range(len(uppacked)):
                # Append new index: last index + 1
                new_index = uppacked[i][-1] + 1
                uppacked[i] = torch.cat([uppacked[i], new_index.unsqueeze(0)], dim=0)
            packed_key_value_indexes = torch.cat(uppacked, dim=0)
        ...
        # Stack into [seq_len, batch_size]
        output_device = generated_sequence[0].device
        return torch.stack([t.to(output_device) for t in generated_sequence], dim=0)  # [seq_len, batch_size]
