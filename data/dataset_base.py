...
class PackedDataset(torch.utils.data.IterableDataset):
    ...
    def build_datasets(self, datasets_metainfo, data_status):
        ...
        for grouped_dataset_name, dataset_args in datasets_metainfo.items():
            ...
            if 'data_dir_list' not in dataset_args.keys():
    ...
    def __iter__(self):
        ...
        buffer = []
        while True:
            # Ensure at least one sample from each group is mandatory
            ...
            if sequence_status['curr'] + num_tokens > self.max_num_tokens:
                if len(buffer) < self.max_buffer_size and not sample_from_buffer:
                    ...
                    print(f"Yielding data with length {sum(sequence_status['sample_lens'])}, num_samples:{len(batch_data_indexes)}")
                    data = self.to_tensor(sequence_status)
                    data['batch_data_indexes'] = batch_data_indexes
                    yield data
                    sequence_status = self.set_sequence_status()
                    batch_data_indexes = []
                    #FIXME will drop the sample
                continue
    ...
    def pack_sequence(self, sample, sequence_status):
        ...
        #if the pack sequence do not has any vit tokens yet, we will avoid the vit drop in this sample
        # if(len(sequence_status['packed_vit_tokens']) == 0):
        #     vit_indices = [
        #         i for i, item in enumerate(sequence_plan)
        #         if item.get('type') == 'vit_image'
        #     ]
        #     if vit_indices:
        #         chosen_index = random.choice(vit_indices)
        #         sequence_plan[chosen_index]['enable_cfg'] = 0
